...
/** Erase a horizontal gradient at the chart's left edge so lines scroll out softly. */
export function drawLeftEdgeFade(ctx: CanvasRenderingContext2D, padLeft: number, h: number): void {
  ctx.save()
...
/** Smoothstep ramp over [start, end] of the reveal progress. */
export function revealRamp(reveal: number, start: number, end: number): number {
...
export function drawFrame(
  ...
  if (opts.showGrid) {
    const gridAlpha = reveal < 1 ? revealRamp(reveal, 0.15, 0.7) : 1
    if (gridAlpha > 0.01) {
      ctx.save()
  ...
  {
    const timeAlpha = reveal < 1 ? revealRamp(reveal, 0.15, 0.7) : 1
    if (timeAlpha > 0.01) {
      ctx.save()
  ...
  if (pts && pts.length > 0) {
    ...
    if (opts.showMomentum) {
      const arrowReveal = reveal < 1 ? revealRamp(reveal, 0.6, 1) : 1
  ...
  drawLeftEdgeFade(ctx, layout.pad.left, layout.h)
...
export function drawCandleFrame(
  ...
  const gridAlpha = revealRamp(reveal, 0.25, 0.6)
  ...
  const closeAlpha = revealRamp(reveal, 0.4, 0.8)
  ...
  const timeAlpha = revealRamp(reveal, 0.25, 0.6)
  if (timeAlpha > 0.01) {
    ctx.save()
    if (timeAlpha < 1) ctx.globalAlpha = timeAlpha
