...
import { drawFrame, drawCandleFrame, drawMultiFrame, drawLeftEdgeFade } from './draw'
...
export interface LivelineEngineOptions {
  canvas: HTMLCanvasElement
  container: HTMLElement
  config: EngineConfig
}

export interface LivelineEngineHandle {
  /** Push a new config snapshot. Cheap — read by the next animation frame. */
  update(config: EngineConfig): void
  /** Tear down listeners, observers, the rAF loop, and the badge DOM node. */
  destroy(): void
}

/**
 * Framework-agnostic Liveline engine. Owns a canvas + container element, runs
 * the requestAnimationFrame draw loop, and renders everything imperatively.
 *
 * The body below is the original React render loop, unchanged. Its three hooks
 * are shimmed locally: `useRef` is a mutable holder, `useCallback` is identity,
 * and `useEffect` registers a setup thunk run once at `mount()`. This keeps the
 * 60fps hot path byte-for-byte identical to the React version while dropping the
 * framework dependency. Adapters stay thin: create elements, instantiate,
 * `update()` on prop change, `destroy()` on unmount.
 */
export function createLivelineEngine(options: LivelineEngineOptions): LivelineEngineHandle {
  const mountCallbacks: Array<() => void | (() => void)> = []
  const cleanups: Array<() => void> = []
  let isMounted = false

  const useRef = <T>(value: T): { current: T } => ({ current: value })
  const useCallback = <T>(fn: T, _deps?: unknown): T => fn
  const useEffect = (setup: () => void | (() => void), _deps?: unknown): void => {
    mountCallbacks.push(setup)
  }

  // Passed-in DOM elements wrapped in the ref shape the body expects.
  const canvasRef: { current: HTMLCanvasElement | null } = { current: options.canvas }
  const containerRef: { current: HTMLElement | null } = { current: options.container }
  ...
  const draw = useCallback(() => {
    ...
    if (!hasData && !useStash && !useMultiStash) {
      ...
      drawLeftEdgeFade(ctx, pad.left, h)
    ...
    const chartW = Math.max(0, w - pad.left - pad.right - labelReserve)
  ...
  // --- imperative lifecycle (replaces React effect scheduling) ---
  const mount = (): void => {
    if (isMounted) return
    isMounted = true
